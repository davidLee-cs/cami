...
uint16_t ti_opt3007_I2C_read(uint8_t registerAddress){
	...
	DL_I2C_startControllerTransfer(I2C_INST, TI_OPT3007_BUSADDRESS, DL_I2C_CONTROLLER_DIRECTION_RX, gRxLen);   // gRxLen :2
...
void ti_opt3007_I2C_write(uint8_t registerAddress,uint16_t value){
	...
	if(value == 0)
	{
		uint16_t length = 1;		// for read mode
	}
	else
	{
		uint16_t length = 3;
...
//	}
	...
	DL_I2C_startControllerTransfer(I2C_INST, TI_OPT3007_BUSADDRESS, DL_I2C_CONTROLLER_DIRECTION_TX, gTxLen);
